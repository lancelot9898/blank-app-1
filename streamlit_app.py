...
membership_plans = {
    ...
    "Platinum": {
        ...
        "workshop": 0,  # Included
        "junior_clinic": 90,
        "junior_discount": 0.4,  # 40% discount
        "monthly_cost": 750
    },
    "Platinum Family": {
...
flex_cost = calculate_cost("FlexPass", selection, junior_sessions)
platinum_cost = calculate_cost("Platinum", selection)
platinum_family_cost = calculate_cost("Platinum Family", selection, junior_sessions)

# Display the costs
st.subheader("Your Monthly Costs")
st.write(f"**FlexPass**: ${flex_cost:.2f} per month")
st.write(f"**Platinum**: ${platinum_cost:.2f} per month")
st.write(f"**Platinum Family**: ${platinum_family_cost:.2f} per month (with junior sessions)")

# Determine the best option
best_option = min(flex_cost, platinum_cost, platinum_family_cost)
